...
对于把最终答案放入 `reasoning_content` 的模型，适配器也会读取该字段；自然
语言中出现多个候选编号时，例如“先考虑 M3，最终选择 M12”，会采用最后一个
编号 M12。适配器将完整棋盘按逐行 JSON 数组提交，并将云端输出上限设置为
4096 tokens；即使模型输出较长分析，也会等待回答结束后再从全文末尾提取
`moveId` 或自然语言坐标。建议搭配 Flash、Chat、Turbo 或 Mini 等非推理模型。
